...
@patch("langraph.app.update_forecasts.load_dotenv")
def test_main_max_concurrency_from_env(mock_dotenv, monkeypatch):
    monkeypatch.setenv("LANGGRAPH_MAX_CONCURRENCY", "1")
    mock_graph = MagicMock()
    mock_graph.invoke.return_value = {}
    test_args = [
        "prog", "--lat", "-36.8", "--lng", "174.7",
    ...
    with (
        patch.object(sys, "argv", test_args),
        patch("langraph.app.graph.build_graph", return_value=mock_graph),
    ):
        main()
        assert mock_graph.invoke.call_args[0][1] == {"max_concurrency": 1}
